...
describe("AddAutoRouterTab", () => {
  ...
  it("creates a member's router with only its name, team and routing configuration", async () => {
    ...
    expect(mockFetchAvailableModels).toHaveBeenLastCalledWith("token", "team-1");
  });

  it("rechecks chosen models when a member changes the destination team and retains the draft", async () => {
    ...
    await waitForPresetEnabled(ANTHROPIC_PRESET.label);
    await selectTemplate(ANTHROPIC_PRESET.label);
    fireEvent.change(screen.getByPlaceholderText(/smart_router/i), { target: { value: "retained-router" } });
    ...
    expect(screen.getByRole("button", { name: /add auto router/i })).toBeDisabled();
    expect(screen.getByPlaceholderText(/smart_router/i)).toHaveValue("retained-router");
    await user.selectOptions(screen.getByTestId("team-dropdown"), "team-1");
    expect(screen.getByRole("button", { name: /add auto router/i })).toBeEnabled();
