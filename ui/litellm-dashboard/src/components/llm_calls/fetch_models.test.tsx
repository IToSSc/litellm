...
describe("fetchAutoRouterModels", () => {
  it("returns no catalog before a destination team has been selected", async () => {
    expect(await fetchAutoRouterModels("token", null)).toEqual([]);
  });
  ...
  it("fails closed when the destination catalog cannot be loaded", async () => {
    modelHubCallMock.mockResolvedValue({ data: [{ model_group: "shared" }] });
