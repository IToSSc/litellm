...
class TestTeamMemberAutoRouterWrites:
    ...
    async def test_key_config_cannot_expand_member_router_targets(self, endpoint: str) -> None:
        from litellm.proxy._types import ProxyException
        from litellm.proxy.management_endpoints.model_management_endpoints import (
            add_new_model,
            patch_model,
        ...
        actor: Final = UserAPIKeyAuth(
            user_id="owner", user_role=LitellmUserRoles.INTERNAL_USER, team_id="member-team",
            models=["personal-router"], config={"timeout": 60},
        )
        ...
        with self._environment(database, row):
            operation: Final = (
                add_new_model(
                    Deployment(
                        model_name="personal-router",
                        litellm_params=LiteLLM_Params(
                            model="auto_router/complexity_router",
                            complexity_router_config={"tiers": {"SIMPLE": "allowed"}},
                        ), model_info=ModelInfo(team_id="member-team"),
                    ), actor,
                )
                if endpoint == "create"
    ...
    @pytest.mark.parametrize("change", ["config", "strategy", "unrelated"])
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("endpoint", ["patch", "legacy"])
    @pytest.mark.parametrize("user_id", ["owner", "peer"])
    ...
    async def test_both_update_entries_enforce_creator_and_stamp_member_scope(
        self, endpoint: str, user_id: str, team_id: str | None
    ) -> None:
        from fastapi import HTTPException
        ...
        request: Final = updateDeployment(
            ...
            model_info=ModelInfo(id=row.model_id) if team_id is None else ModelInfo(id=row.model_id, team_id=team_id),
        )
        actor: Final = UserAPIKeyAuth(user_id=user_id, user_role=LitellmUserRoles.INTERNAL_USER, models=["allowed"])
        with self._environment(database, row):
    ...
    async def test_create_entry_requires_opt_in_and_appends_only_its_router(self, enabled: bool) -> None:
        from litellm.proxy._types import ProxyException
        from litellm.proxy.management_endpoints.model_management_endpoints import add_new_model

        row: Final = self._row()
        database: Final = self._database(self._team(enabled=enabled), row)
        actor: Final = UserAPIKeyAuth(user_id="owner", user_role=LitellmUserRoles.INTERNAL_USER, models=["allowed"])
