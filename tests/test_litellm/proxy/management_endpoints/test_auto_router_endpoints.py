...
async def test_member_preview_and_validation_follow_team_opt_in(
    ...
    actor: Final = _configure_member_preview(monkeypatch, allowed=allowed)
    monkeypatch.setattr(proxy_server, "llm_router", _router())
    ...
    if not allowed:
        with pytest.raises(HTTPException) as denied_preview:
            await preview_auto_router_routing(preview, actor, ROUTING_HTTP_REQUEST)
        with pytest.raises(HTTPException) as denied_validation:
            await validate_complexity_router_config(validation, actor)
        assert denied_preview.value.status_code == denied_validation.value.status_code == 403
        return
...
@pytest.mark.asyncio
@pytest.mark.parametrize("unsafe", ["target", "override"])
async def test_member_dry_runs_reject_inaccessible_models_and_egress_overrides(
    ...
    actor: Final = _configure_member_preview(monkeypatch)
    monkeypatch.setattr(proxy_server, "llm_router", _router())
    tiers: Final = {
        "SIMPLE": (
            "missing-model"
            if unsafe == "target"
            else [{"model_name": "cheap-model", "litellm_params": {"api_base": "https://invalid.example"}}]
        )
    }
    config: Final = {"tiers": tiers, "classifier_type": "heuristic"}
    with pytest.raises(HTTPException) as denied_validation:
        await validate_complexity_router_config(
            ComplexityRouterConfigValidationRequest(team_id="member-preview-team", complexity_router_config=config), actor
        )
    with pytest.raises(HTTPException) as denied_preview:
        await preview_auto_router_routing(
            _request_from({"prompt": "hi", "team_id": "member-preview-team"}, **config), actor, ROUTING_HTTP_REQUEST
        )
    assert denied_validation.value.status_code == denied_preview.value.status_code == 400


@pytest.mark.asyncio
async def test_member_preview_key_config_cannot_expand_classifier_model_access(monkeypatch: pytest.MonkeyPatch) -> None:
    from litellm.proxy import proxy_server

    actor: Final = _configure_member_preview(monkeypatch).model_copy(update={
