...
async def test_unknown_and_legacy_turns_preserve_actual_spend_without_entering_the_estimated_cohort(db: Prisma) -> None:
    ...
    row: Final = await _row(db, key)
    assert row["turns"] == 3
    assert row["spend"] == pytest.approx(0.96)
    assert row["saved_spend"] == pytest.approx(-0.03)
    assert row["savings_estimated_turns"] == 1
    assert row["savings_estimated_actual_spend"] == pytest.approx(0.25)
    assert row["savings_estimated_saved_spend"] == pytest.approx(-0.05)
    ...
    assert len(groups) == 1
    assert groups[0]["turns"] == 3
    assert groups[0]["spend"] == pytest.approx(0.96)
    assert groups[0]["savings_estimated_turns"] == 1
    assert groups[0]["savings_estimated_actual_spend"] == pytest.approx(0.25)
    assert groups[0]["savings_estimated_saved_spend"] == pytest.approx(-0.05)
