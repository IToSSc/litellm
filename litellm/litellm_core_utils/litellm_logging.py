...
class Logging(LiteLLMLoggingBaseClass):
    ...
    async def _prepare_baseline_cache_estimate(self, response_obj: object) -> None:
        context: Final = self.baseline_cache_context
        if context is None:
            return
        try:
        ...
        except Exception:  # noqa: BLE001  # estimation must not prevent spend logging
            from litellm.proxy.spend_tracking.autorouter_baseline_cache import unknown_estimate

            if self.baseline_cache_context is context:
                self.baseline_cache_estimate = unknown_estimate("estimator_unavailable")
            try:
                from litellm.proxy.hooks.autorouter_baseline_cache import invalidate_baseline_cache

                await invalidate_baseline_cache(self, "estimator_unavailable", completed=True, expected_context=context)
            except Exception:  # noqa: BLE001  # cleanup faults must not prevent spend logging
                return

    async def invalidate_baseline_cache_estimate(self, reason: str) -> None:
        """A retried request has no single completed wire request to model."""
        from litellm.proxy.hooks.autorouter_baseline_cache import invalidate_baseline_cache

        await invalidate_baseline_cache(self, reason)
            ...
            **(
            ...
            autorouter_savings_estimate=(
                ...
                if not metadata.get("routing_decision")
                    ...
                    if prepared_estimate is not None and autorouter_savings is not None
