...
def _positions(body: Mapping[str, JsonValue]) -> tuple[_Position, ...]:
    ...
    return (
        *tuple(
            _position("tools", -1, index, "", _JSON_OBJECT.validate_python(tool))
            for index, tool in enumerate(tools if isinstance(tools, list) else ())
        ),
        *tuple(
            _position("system", -1, index, "", block) for index, block in enumerate(_content_blocks(body.get("system")))
        ),
        *tuple(
            _position("messages", message_index, block_index, str(message.get("role")), block)
...
def parse_cache_plan(body: Mapping[str, JsonValue]) -> PromptCachePlan | UnsupportedCachePlan:
    ...
    settings: Final = MappingProxyType(
        {key: body[key] for key in ("thinking", "output_config", "speed") if key in body}
    )
    hashes: Final = tuple(
        accumulate(
            (
...
async def count_cache_plan(
    ...
    if any(position.block.get("type") == "thinking" for position in _positions(plan.full_body)):
        retention: Final = re.search(r"(?:^|/)claude-(opus|sonnet)-(\d+)(?:[-.](\d+))?", model)
        if retention is None or not (
            int(retention[2]) >= 5
            or (int(retention[2]) == 4 and int(retention[3] or 0) >= (5 if retention[1] == "opus" else 6))
        ):
